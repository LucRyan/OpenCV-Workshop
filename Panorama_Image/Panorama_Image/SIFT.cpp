...
mySIFT::mySIFT(IplImage* img, int octaves, int intervals)
{
...
// cutoff value.
unsigned int mySIFT::GetKernelSize(double sigma, double cut_off)
{
    unsigned int i;
...
// addition earlier.
CvMat* mySIFT::BuildInterpolatedGaussianTable(unsigned int size, double sigma)
{
	unsigned int i, j;
...
void mySIFT::displayDecs(){
    for(int i=0;i<m_keyDescs.size();i++)
	{
        printf("X: %f\t", m_keyDescs.at(i).xi);
        printf("Y: %f\t", m_keyDescs.at(i).yi);
        cout << endl;
        cout << "Features: " << endl;
        for(int j=0; j<m_keyDescs.at(i).fv.size(); j++)
        {
            printf("%f\t", m_keyDescs.at(i).fv.at(j));
        }
        printf("\n");
	}
}

CvMat mySIFT::getDescriptors(){
